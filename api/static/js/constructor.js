...
function saveInputs() {
    const blockConfig = [
        ...
        {selector: "input[name^='LinkBlock_'][name$='_text']", name: "LinkBlock_text_new"},
        {selector: "input[name^='ImageBlock_'][name$='_image']", name: "ImageBlock_image_new"},
        {selector: "input[name^='TextBlock_QUOTETEXT_']", name: "TextBlock_QUOTETEXT_new"},
        {selector: "input[name^='DividerBlock_']", name: "DividerBlock_new"},
        {selector: "input[name^='TextBlock_HEADERTEXT_']", name: "TextBlock_HEADERTEXT_new"},
    ];

    const new_blocks_ids = document.getElementById("new_blocks_ids");
    const main = document.querySelector("main");

    // Очистка содержимого new_blocks_ids перед добавлением новых элементов
    new_blocks_ids.innerHTML = "";

    // Создание и добавление скрытых input элементов
    blockConfig.forEach(config => {
        const input = document.createElement("input");
        input.type = "hidden";
        input.value = main.querySelectorAll(config.selector).length;
        input.name = config.name;
        new_blocks_ids.appendChild(input);
    });

    var inputs = main.querySelectorAll("input, textarea");
    var filledInputs = Array.from(inputs).filter(input => !input.name.trim().endsWith("_new"));
